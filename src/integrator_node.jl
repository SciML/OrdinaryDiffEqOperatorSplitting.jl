"""
    OperatorSplittingIntegratorNode <: AbstractODEIntegrator
...
"""
mutable struct OperatorSplittingIntegratorNode{
        fType,
        algType,
        uType,
        tType,
...
function reject_step!(integrator::OperatorSplittingIntegrator)
    ...
    return reject_step!(integrator, integrator.controller)
end
function reject_step!(integrator::OperatorSplittingIntegrator, controller)
    ...
    return prepare_subintegrators_to_redo_step!(integrator)
end
    ...
    while !isempty(integrator.tstops)
        while tdir(integrator) * integrator.t < SciMLBase.first_tstop(integrator)
            ...
            @timeit_debug "check_error" DiffEqBase.check_error!(integrator) ∉ (
                SciMLBase.ReturnCode.Success, SciMLBase.ReturnCode.Default,
            )&&return
            __step!(integrator)
...
@inline function stepsize_controller!(integrator::OperatorSplittingIntegrator)
    ...
    return stepsize_controller!(integrator, integrator.controller, integrator.alg)
end
...
@inline function step_accept_controller!(integrator::OperatorSplittingIntegrator)
    DiffEqBase.isadaptive(integrator) || return nothing
    return step_accept_controller!(integrator, integrator.controller, integrator.alg)
end
...
@inline function step_reject_controller!(integrator::OperatorSplittingIntegrator)
    DiffEqBase.isadaptive(integrator) || return nothing
    return step_reject_controller!(integrator, integrator.controller, integrator.alg)
end
...
function synchronize_subintegrator!(
    ...
    return if !DiffEqBase.isadaptive(subintegrator)
